  ...
  const handleDeleteBook = () => {
    try {
      startTransition(async () => {
        ...
        if (response.success) {
          toast.success(resolveActionMessage(response.messageKey, translations || {} as LocaleDict));
          setIsOpen(false);
          window.location.href = '/';
        } else {
          console.error('Error deleting book:', response?.messageKey);
          toast.error(resolveActionMessage(response?.messageKey || 'actions.book.unexpectedError', translations || {} as LocaleDict));
        }
      });
    } catch (error) {
      console.error('Error deleting book:', error);
      toast.error(resolveActionMessage('actions.book.unexpectedError', translations || {} as LocaleDict));
    }
  }
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="w-full md:min-w-[350px]">
              ...
              <DialogFooter className="sm:justify-end">
                ...
                >
                  {isPending ? translations.page.bookDetails.deleting : translations.page.bookDetails.deleteBook}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
